...
// 영화 데이터를 가져오는 함수
const fetchUpcomingPlayingMovies = async ({ pageParam = 1 }) => {
  const response = await axios.get(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${myAPIkey}&language=ko-KR&page=${pageParam}`
  );
...
const upcomingPlaying = () => {
  ...
  const {
    ...
    queryKey: ['UpcomingPlayingMovies'],
