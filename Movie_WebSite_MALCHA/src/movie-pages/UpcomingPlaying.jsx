...
const fetchUpcomingPlayingMovies = async () => {
  const response = await axios.get(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${myAPIkey}&language=ko-KR&page=1`
  );
...
const UpcomingPlaying = () => {
  ...
  const { data: movies, isLoading, isError, error } = useQuery({
    queryKey: ['UpcomingPlayingMovies'], // queryKey를 설정
    queryFn: fetchUpcomingPlayingMovies,  // API 호출 함수
  });
