...
// 영화 데이터를 가져오는 함수
const fetchPopularPlayingMovies = async () => {
  const response = await axios.get(
    `https://api.themoviedb.org/3/movie/popular?api_key=${myAPIkey}&language=ko-KR&page=1`
  );
...
const PopularPlaying = () => {
  ...
  const { data: movies, isLoading, isError, error } = useQuery({
    queryKey: ['popularPlayingMovies'], // queryKey를 설정
    queryFn: fetchPopularPlayingMovies,  // API 호출 함수
  });
