...
// 영화 데이터를 가져오는 함수
const fetchPrizedPlayingMovies = async ({ queryKey }) => {
  const [_key, { page, language }] = queryKey;
  const response = await axios.get(
    `https://api.themoviedb.org/3/movie/top_rated?api_key=${myAPIkey}&language=${language}&page=${page}`
  );
...
const PrizedPlaying = () => {
  ...
  const { 
    ...
    queryKey: ['PrizedPlayingMovies', { page: currentPage, language: 'ko-KR' }], // queryKey에 페이지와 언어를 명확히 포함
